...
public class MapCameraController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        else if (Input.GetMouseButton(0))
        {
            ...
            transform.position = new Vector3(transform.position.x - (MouseMove - mouseStart).x, transform.position.y-(MouseMove-mouseStart).y,-10);
            ...
            if(transform.position.y >= MaxPosTop)
            {
                transform.position = new Vector3(transform.position.x, MaxPosTop, -10);
            }
            else if(transform.position.y <= MaxPosDown)
            {
                transform.position = new Vector3(transform.position.x, MaxPosDown, -10);
            }
        }
